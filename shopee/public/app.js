...
async function apiGet(path) {
  // Garantir prefixo /shopee
  if (path.startsWith("/") && !path.startsWith("/shopee")) {
    path = "/shopee" + path;
  }
        ...
        return `
          <div class="relaunch-section">
                      ...
                      .map((p) => {
                        ...
                        const rating =
                          p.ratingStar != null
                            ...
                            : "—";
                        return `
                          <div class="relaunch-product-row">
            ...
            <div class="relaunch-section__footer">
              <button class="relaunch-action-btn" onclick="relaunchSelectAll(this)">Selecionar tudo</button>
              <button class="relaunch-action-btn" onclick="relaunchDeselectAll(this)">Desselecionar</button>
              <button class="relaunch-action-btn" onclick="relaunchPauseSelected(this)">Pausar selecionados</button>
              <button class="relaunch-action-btn danger" onclick="relaunchDeleteSelected(this)">Excluir anúncios</button>
            </div>
          </div>
        `;
      })
...
async function relaunchPauseSelected(btn) {
  ...
  // TODO: Implementar pausa de anúncios
  alert(`Pausar ${RELAUNCH_SELECTED_IDS.size} anúncios (em desenvolvimento)`);
}
...
async function relaunchDeleteSelected(btn) {
  ...
  if (
    !confirm(
      `Você tem certeza que deseja excluir ${RELAUNCH_SELECTED_IDS.size} anúncios? Download do XLS será iniciado automaticamente.`,
    )
  ) {
    return;
  }
  // TODO: Implementar exclusão e download XLS
  alert(`Excluir ${RELAUNCH_SELECTED_IDS.size} anúncios (em desenvolvimento)`);
}
...
function initMargin() {
  document
    .getElementById("btnLoadMargin")
    ?.addEventListener("click", loadMargin);
  const now = new Date();
  const start = new Date();
  start.setDate(1); // 1o do mês
...
async function loadMargin() {
  ...
  try {
    const data = await apiGet(
      `/dashboard/margin?dateFrom=${dateFrom}&dateTo=${dateTo}`,
    );

    setText("marginRevenue", formatBRLFixed90(data.revenue));
    setText("marginTotalCosts", formatBRLFixed90(data.costs.total));
    setText("marginProfit", formatBRLFixed90(data.profit));
    setText(
      "marginProfitPercent",
      (Number(data.profitMargin) || 0).toFixed(2) + "%",
    );

    setText("marginCommissions", formatBRLFixed90(data.costs.commissions));
    setText("marginProductCosts", formatBRLFixed90(data.costs.products));
    setText("marginReturns", formatBRLFixed90(data.costs.returns));
    setText("marginTaxes", formatBRLFixed90(data.costs.taxes));
    setText("marginAds", formatBRLFixed90(data.costs.ads));
    setText("marginShipping", formatBRLFixed90(data.costs.shipping));
    setText("marginVouchers", formatBRLFixed90(data.costs.vouchers));

    if (data.spx) {
      setText("marginSpxCost", formatBRLFixed90(data.spx.totalCents));
      setText("marginSpxCount", (data.spx.count || 0) + " pedidos");
    }
  } catch (e) {
    console.error(e);
    alert("Erro ao carregar margem: " + e.message);
  }
}
...
function initCosts() {
  ...
  document.getElementById("btnExportCosts")?.addEventListener("click", () => {
    window.open("/shopee/shops/active/costs/export", "_blank");
  });
  ...
  document
    ...
    ?.addEventListener("change", async (e) => {
      ...
      // Ler CSV localmente e enviar JSON
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Ex: "123","Nome, com virgula",SKU,10.50
        ...
        while ((match = regex.exec(line))) {
          // match[1] é o valor sem aspas (se estava entre aspas)
          // match[2] é o valor sem aspas (se não tinha aspas)
        ...
        // Tratamento Científico: 2,28E+10 -> 2.28E+10 -> 22800000000
        if (itemIdRaw.toUpperCase().includes("E+")) {
          // Se vier com vírgula (2,28E+10), troca pra ponto pra o JS entender
          itemIdRaw = itemIdRaw.replace(",", ".");
          const f = parseFloat(itemIdRaw);
          if (!isNaN(f)) {
            // Converte para string inteira completa
            itemIdRaw = f.toLocaleString("fullwide", { useGrouping: false });
          }
        }
        // Remove aspas extras se sobrarem e espaços
        const itemId = itemIdRaw.replace(/["']/g, "");

        // Custo
        let costStr = cols[3];
        // Se vier R$ 1.200,50 -> remove R$, remove . milhar, troca , por .
        costStr = costStr.replace("R$", "").trim();

        // Se tiver separador de milhar e decimal (ex: 1.200,50)
        // Regra simples: se tem , e . e o . vem antes, é formato US (1,200.50).
        // Se o . vem antes da , (1.200,50) é BR.
        // Mas assumindo planilha PT-BR:
        if (costStr.includes(",") && costStr.includes(".")) {
          // remove ponto (milhar) e troca virgula por ponto
...
async function loadCosts() {
  ...
  try {
    const data = await apiGet(
      `/shops/active/costs?page=${COSTS_PAGE}&pageSize=${COSTS_PAGE_SIZE}&q=${encodeURIComponent(search)}`,
    );
