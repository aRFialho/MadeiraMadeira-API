...
function renderCpcProductPerformanceTable(items) {
  ...
  for (const it of lastCpcProductPerfRows) {
    ...
    // Custo por Venda = Expense / items (ou conversions)
    // O usuário pediu "dividido pela quantidade de vendas por ads" (que é 'items' ou 'directSold').
    // Vamos usar 'items' que é a quantidade vendida.
    const expense = Number(it.expense || 0);
    const sold = Number(it.items || 0);
    const costPerSale = sold > 0 ? expense / sold : 0;
