...
class KeywordAnalyticsService {
  ...
  static async _mlGet(path, qs = {}, headers = {}) {
    ...
    try {
      ...
      console.log("KA ML response", {
        endpoint: `${path}${url.search ? url.search : ""}`,
        status: r.status,
    ...
    if (!r.ok) {
      ...
      err.endpoint = `${path}${url.search ? url.search : ""}`;
  ...
  static async _fetchItemPublic(itemId) {
    const cleanId = String(itemId || "").trim().toUpperCase();

    try {
      return await this._mlGet(`/items/${encodeURIComponent(cleanId)}`, {}, {});
    } catch (err) {
      if (![401, 403, 404].includes(Number(err?.statusCode))) throw err;

      console.warn("KA item fetch fallback", {
        itemId: cleanId,
        fromEndpoint: err?.endpoint || `/items/${cleanId}`,
        status: err?.statusCode || null,
      });

      const multi = await this._mlGet(
        "/items",
        { ids: cleanId },
        {},
      );

      const first = Array.isArray(multi) ? multi[0] : null;
      if (Number(first?.code) === 200 && first?.body) {
        return first.body;
      }

      const fallbackErr = new Error(
        first?.body?.message || first?.message || err?.message || "Falha ao buscar item",
      );
      fallbackErr.statusCode = Number(first?.code) || err?.statusCode || 500;
      fallbackErr.endpoint = `/items?ids=${encodeURIComponent(cleanId)}`;
      fallbackErr.payload = first?.body || first || err?.payload || null;
      throw fallbackErr;
    }
  }
  ...
  static async computeSuggestions({
    ...
    if (!mlCreds?.meli_conta_id) {
      const err = new Error("Conta Mercado Livre não selecionada");
      err.statusCode = 401;
      err.code = "ML_AUTH_INVALID";
      err.endpoint = "/api/account/current";
      throw err;
    }

    try {
      await TokenService.renovarTokenSeNecessario(mlCreds || {});
    } catch (e) {
    ...
    try {
      if (mlCreds && mlCreds.access_expires_at) {
        tokenAgeSeconds = Math.round(
          (new Date(mlCreds.access_expires_at).getTime() - Date.now()) / 1000,
        );
      }
    } catch {}
    ...
    const item = await this._fetchItemPublic(item_id);
    ...
    if (item?.seller_id && String(item.seller_id) !== String(t.user_id)) {
      ...
      err.details = {
        ...
        account_seller_id: t.user_id,
      };
      throw err;
    }

    const resolvedSite = String(item?.site_id || site || "MLB")
      .trim()
      .toUpperCase();
    ...
    console.log("KA ML request (pre)", {
      ...
      hasAccessToken,
      tokenAgeSeconds,
      endpoint: `/sites/${encodeURIComponent(resolvedSite)}/search?q=${encodeURIComponent(seedUsed)}&limit=50&offset=0`,
    });

    const serpTitles = seedUsed
      ? await this._serpTitles({ site: resolvedSite, seed: seedUsed, headers: {} })
      : [];

    const ngrams = this._countFreq(this._ngrams(this._toks(serpTitles.join(" "))));
    ...
    console.log("KA ML request (pre)", {
      ...
      hasAccessToken,
      tokenAgeSeconds,
      endpoint: `/trends/${encodeURIComponent(resolvedSite)}`,
    });

    const trends = await this._mlGet(`/trends/${encodeURIComponent(resolvedSite)}`, {}, {});
    const trendKeywords = Array.isArray(trends?.trends)
      ? trends.trends
          .map((tt) => String(tt?.keyword || tt?.value || "").trim())
          .filter(Boolean)
    ...
    for (const kw of dedup) {
      ...
      const relevance = this._jaccard(this._toks(kw), [...baseToks, ...attrToks]);
      ...
      out.push({
        ...
        rank_bucket: rankInfo.position ? `#${rankInfo.position}` : `> ${pages * 50}`,
        competition,
        trend: trendScore,
        relevance,
        score,
    ...
    const data = {
      item: {
        id: item?.id || item_id,
        site_id: item?.site_id || resolvedSite,
