...
// Remove um único item chamando diretamente o serviço (evita problemas de base /ml)
async function removeOneDirect({ mlb, mlCreds }) {
  try {
    const res = await PromocaoService.removerPromocaoUnico(mlb, { mlCreds });
    return {
      ok: !!res?.success,
...
async function removeOne({ mlCreds, promotion_id, promotion_type, item_id }) {
  try {
    const res = await removeOneDirect({ mlb: item_id, mlCreds });
    return {
      ok: !!res.ok,
  ...
  (async () => {
    try {
      ...
      for (const mlb of mlbIds) {
        ...
        try {
          const res = await removeOneDirect({ mlb, mlCreds });
          if (res.ok) job.ok++;
          else job.err++;
        } catch (e) {
          job.err++;
