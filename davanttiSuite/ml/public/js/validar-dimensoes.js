...
(() => {
  ...
  async function verificarToken(updateModal = false) {
    try {
      const response = await fetch(withBase('/api/tokens/verificar-token'));
  ...
  async function renovarToken(updateModal = false) {
    try {
      const response = await fetch(withBase('/api/tokens/renovar-token-automatico'), {
        method: 'POST',
      });
      const data = await response.json();
      if (data.success) {
  ...
  async function fetchDimensoes(mlb) {
    try {
      const res = await fetch(withBase('/api/validar-dimensoes/analisar-item'), {
        method: 'POST',
