...
const JobsWatcher = (function () {
  ...
  function normalizeJobState(job) {
    ...
    const rawProgress = Number(job?.progress ?? NaN);
    ...
    const errors = Number(
      job?.errors ?? job?.error_count ?? job?.failed ?? job?.failures ?? 0,
    );
    const hasErrors = Number.isFinite(errors) && errors > 0;
    const hasErrorsFromText = /\b(failed|fail|falhou|error|erro)\b/i.test(sRaw);
  ...
  async function poll() {
    try {
      const r = await fetch(withBase("/api/promocoes/jobs"), {
        credentials: "same-origin",
        cache: "no-store",
      ...
      console.error("[JobsWatcher.poll] erro ao atualizar jobs:", e);
    }
  }
  ...
  try {
    ...
    if (t === "SELLER_CAMPAIGN" || t === "DEAL") {
      ...
      if (!candidateId || !offerId) {
        const { candidateId: c2, offerId: o2 } =
          await buscarOfferIdCandidate(mlb);
...
async function aplicarTodosFiltrados() {
  ...
  try {
    ...
    const prepRes = await fetch(withBase("/api/promocoes/selection/prepare"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
