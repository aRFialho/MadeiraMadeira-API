(function () {
  ...
  async function trocarConta() {
    const withBase = window.mlUrl ? window.mlUrl : (p) => p;

    try {
      await fetch(withBase("/api/account/clear"), { method: "POST" });
    } catch (_) {}
    updateJobIdInUrl(null);
    window.location.href = withBase("/select-conta");
  }

  function abrirStatusRapido() {
    const withBase = window.mlUrl ? window.mlUrl : (p) => p;
    window.location.href = withBase("/dashboard#status");
  }
