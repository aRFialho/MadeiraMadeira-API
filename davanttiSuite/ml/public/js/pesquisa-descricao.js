...
async function executarPesquisaRapida() {
  ...
  try {
    ...
    const resp = await fetch(withBase('/api/pesquisa-descricao/enfileirar'), { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
...
async function atualizarMonitoramento() {
  try {
    // Stats
    const st = await fetch(withBase('/api/pesquisa-descricao/status?_=') + Date.now(), { cache: 'no-store' });
    ...
    // Jobs
    const jb = await fetch(withBase('/api/pesquisa-descricao/jobs?_=') + Date.now(), { cache: 'no-store' });
  ...
  return `
    <div class="job-card" data-job-id="${id}" data-status="${status}" data-total="${total}" data-done="${done}">
      ...
      <div class="actions">
        ...
        ${status==='concluido' ? `<a href="${withBase('/api/pesquisa-descricao/download/' + id)}" class="btn btn-success">Download Resultados</a>` : ''}
      </div>
    </div>
  `;
}
...
async function verDetalhesJob(jobId) {
  ...
  try {
    const resp = await fetch(withBase('/api/pesquisa-descricao/jobs?_=') + Date.now(), { cache:'no-store' });
    const data = await resp.json();
    ...
    content.innerHTML = `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <a href="${withBase('/api/pesquisa-descricao/download/' + jobId)}?formato=txt" class="btn btn-success">📥 Download (.txt)</a>
        <a href="${withBase('/api/pesquisa-descricao/download/' + jobId)}" class="btn btn-outline">JSONL</a>
...
async function verResultados(jobId) {
  currentJobId = jobId;
  try {
    const resp = await fetch(withBase(`/api/pesquisa-descricao/jobs/${jobId}?_=`) + Date.now(), { cache:'no-store' });
    ...
    content.innerHTML = `
      <div style="text-align:center;padding:40px;">
        ...
        <div style="margin-top:30px;">
          ...
          <button class="btn btn-success" onclick="window.location.href = withBase('/api/pesquisa-descricao/download/')+(window.currentJobId||'')+'?formato=txt'">📥 Download Resultados Completos</button>
        </div>
      </div>
    `;
  }
