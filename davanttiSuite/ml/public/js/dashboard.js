...
async function trocarConta() {
  ...
  // e respeita base path (/ml) automaticamente
  const to = window.mlUrl ? window.mlUrl : (p) => p;
  try {
    await fetch(to("/api/meli/limpar-selecao"), {
      method: "POST",
      credentials: "include",
    });
  } catch {}
...
async function verificarToken(updateModal = false) {
  try {
    const response = await fetch("/api/tokens/verificar-token", {
      credentials: "include",
    });
...
async function renovarToken(updateModal = false) {
  try {
    const response = await fetch("/api/tokens/renovar-token-automatico", {
      method: "POST",
      credentials: "include",
    });
