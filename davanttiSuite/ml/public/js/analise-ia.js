...
(() => {
  ...
  function renderDiagnostic(data) {
    ...
    const vt = Number(data?.visits?.total ?? 0);
    const sd = Number(s?.sold_quantity ?? 0);
    const conv = vt > 0 ? Math.min(100, Math.max(0, (sd / vt) * 100)) : null;
    const resumo = ""; // será renderizado como cards
    const positivos = [];
    if (s?.is_premium) positivos.push("Anúncio Premium ativo");
    if (ship?.free_shipping) positivos.push("Frete grátis ativo");
    if (rep?.power_seller_status)
      positivos.push(`Power seller: ${rep.power_seller_status}`);
    if (rep?.level_id) positivos.push(`Reputação ${rep.level_id}`);
    const problemas = [];
    if (!s?.catalog_listing) problemas.push("Fora de catálogo");
    if (!ship?.free_shipping) problemas.push("Sem frete grátis");
    if (conv != null && conv < 5) problemas.push("Conversão baixa");
    const recomendacoes = [];
    if (!s?.catalog_listing) recomendacoes.push("Entrar no catálogo");
    if (!ship?.free_shipping)
      recomendacoes.push("Oferecer frete grátis (itens estratégicos)");
    if (conv != null && conv < 5)
      recomendacoes.push(
        "Revisar título, imagens e preço para melhorar conversão",
      );
    const alertas = [];
    if (rank?.position == null)
      alertas.push("Ranking não disponível para a busca estimada");
    if (!rep?.metrics) alertas.push("Métricas de reputação incompletas");
    const statCards = `
      <div class="stat-cards">
        <div class="stat-card"><div class="value">${fmtMoneyBRL(s?.price)}</div><span class="label">Preço</span></div>
        <div class="stat-card"><div class="value">${fmtInt(s?.available_quantity)}</div><span class="label">Estoque</span></div>
        <div class="stat-card"><div class="value">${fmtInt(s?.sold_quantity)}</div><span class="label">Vendidos</span></div>
        <div class="stat-card ${conv != null && conv < 5 ? "warn" : "ok"}"><div class="value">${conv != null ? fmtPct(conv / 100) : "—"}</div><span class="label">Conversão</span></div>
        <div class="stat-card ${s?.is_premium ? "ok" : "warn"}"><div class="value">${s?.is_premium ? "Premium" : "Clássico"}</div><span class="label">Tipo</span></div>
        <div class="stat-card ${ship?.free_shipping ? "ok" : "bad"}"><div class="value">${ship?.free_shipping ? "Grátis" : "Pago"}</div><span class="label">Frete</span></div>
        <div class="stat-card ${s?.catalog_listing ? "ok" : "bad"}"><div class="value">${s?.catalog_listing ? "Catálogo" : "Fora"}</div><span class="label">Catálogo</span></div>
      </div>
    `;
    const html = `
      <div class="diag-title">📊 <span>Diagnóstico rápido</span></div>
      ${statCards}
      <div class="analysis-grid">
        <div class="analysis-card ok">
          <div class="card-title">✅ Pontos Positivos</div>
          <ul class="card-list">
            ${positivos.length ? positivos.map((x) => `<li>${escapeHtml(x)}</li>`).join("") : `<li>—</li>`}
          </ul>
        </div>
        <div class="analysis-card warn">
          <div class="card-title">⚠ Problemas Identificados</div>
          <ul class="card-list">
            ${problemas.length ? problemas.map((x) => `<li>${escapeHtml(x)}</li>`).join("") : `<li>—</li>`}
          </ul>
        </div>
      </div>
      <div class="diag-title" style="margin-top:8px;">💡 <span>Recomendações</span></div>
        ...
        ${
          recomendacoes.length
            ? recomendacoes
                .map(
                  (r) =>
                    `<div class="action-card"><div class="title">💡 ${escapeHtml(
                      r,
                    )}</div><div class="desc">Aplique quando fizer sentido comercialmente.</div></div>`,
                )
                .join("")
            : `<div class="action-card"><div class="title">Sem recomendações</div><div class="desc">O anúncio parece consistente nos critérios básicos.</div></div>`
        }
      </div>
    `;
    if (el.diagBox) el.diagBox.innerHTML = html;
  }
  ...
  function renderRanking(data) {
    ...
    if (!r || r.overall == null) {
      const pos =
        rank?.position != null
          ? `Posição estimada: #${fmtInt(rank.position)} de ${fmtInt(
              rank.total ?? 0,
            )}`
          : "Posição estimada: —";
      el.rankBox.innerHTML = `<div class="text-muted">${escapeHtml(pos)}</div>`;
      return;
    }
    const score = Number(r.overall);
    const cls = score >= 80 ? "Excelente" : score >= 60 ? "Bom" : "Ruim";
    const barColor = (v) =>
      v >= 80 ? "#16a34a" : v >= 60 ? "#f59e0b" : "#dc2626";
    const rows = [
      ["SEO", br.seo],
      ["Estrutura da descrição", br.estrutura_descricao],
      ["Clareza do título", br.clareza_titulo],
      ["Palavras‑chave", br.palavras_chave],
      ["Escaneabilidade", br.escaneabilidade],
      ["Informações técnicas", br.informacoes_tecnicas],
    ]
      .map(
        ([k, v]) =>
          `<div class="score-row">
             <div class="k">${escapeHtml(String(k))}</div>
             <div class="score-bar"><span style="width:0%" data-w="${Number(v || 0)}" data-c="${barColor(Number(v||0))}"></span></div>
             <div class="v">${v != null ? Number(v) : "—"}</div>
           </div>`,
      )
      .join("");
    const posLine =
      rank?.position != null
        ...
        : null;
    const tone =
      score >= 80 ? "score-good" : score >= 60 ? "score-warn" : "score-bad";
    el.rankBox.innerHTML = `
      <div class="diag-title">🏆 <span>Rankeamento do anúncio</span></div>
      <div class="scorecard ${tone}">
        <div class="score-line">
          <div class="score-big">${Number(score)}/100</div>
          <div class="score-label">${escapeHtml(cls)}</div>
        </div>
        <div class="score-breakdown">
          ${rows}
        </div>
        ${posLine ? `<div class="text-muted" style="margin-top:6px;">${escapeHtml(posLine)}</div>` : ""}
      </div>`;
    // micro-animação: preenche as barras após o render
    try {
      const bars = el.rankBox.querySelectorAll(".score-bar > span");
      requestAnimationFrame(() => {
        bars.forEach((sp) => {
          const w = sp.getAttribute("data-w");
          const c = sp.getAttribute("data-c");
          if (c) sp.style.background = c;
          if (w != null) sp.style.width = `${Number(w)}%`;
        });
      });
    } catch {}
  }
  ...
  function renderNegatives(data) {
    if (!el.negativesBox) return;
    const list = Array.isArray(data?.metrics?.negatives)
      ? data.metrics.negatives
      : [];
    if (!list.length) {
      el.negativesBox.textContent = "—";
      return;
    }
    el.negativesBox.innerHTML = `
      <ul class="card-list">
        ${list.map((x) => `<li>${escapeHtml(String(x))}</li>`).join("")}
      </ul>
    `;
  }
  ...
  function renderAiInsights(ai) {
    ...
    const resumo = ai.resumo || "";
    const positivos = Array.isArray(ai.positivos) ? ai.positivos : [];
    const problemas = Array.isArray(ai.problemas) ? ai.problemas : [];
    const recomendacoes = Array.isArray(ai.recomendacoes)
      ? ai.recomendacoes
      : [];
            ...
            .map((x) => {
              ...
              return `
                <div class="alert-card">
                  <div class="head"><span class="alert-badge">${escapeHtml(sev)}</span> ${escapeHtml(tipo)}</div>
                  <div class="meta">${escapeHtml(what)}</div>
                  ${act ? `<div class="meta"><b>Ação:</b> ${escapeHtml(act)}</div>` : ""}
                  ${impact ? `<div class="meta"><b>Impacto:</b> ${escapeHtml(impact)}</div>` : ""}
                </div>`;
            })
            .join("")}
        </div>`
      : `<div class="muted" style="margin-top:6px;">Sem insights (a IA pode ter retornado missing_data).</div>`;

    const sec = (title, arr) =>
      `<div class="analysis-card"><div class="card-title">${escapeHtml(
        title,
    ...
    const actionsHtml =
      Array.isArray(recomendacoes) && recomendacoes.length
        ? `<div class="action-cards">${recomendacoes
            .map((r) => {
              const s = String(r || "").toLowerCase();
              const cls = s.includes("frete")
                ? "action-green"
                : s.includes("convers")
                  ? "action-orange"
                  : "action-blue";
              return `<div class="action-card ${cls}"><div class="title">💡 ${escapeHtml(
                r,
              )}</div><div class="desc">Aplique quando fizer sentido comercialmente.</div></div>`;
            })
            .join("")}</div>`
    ...
    el.aiDiagBox.innerHTML = `
      <div class="diag-title">🧠 <span>IA • Insights</span></div>
      <div class="text-muted" style="margin-top:2px;">${escapeHtml(scoreLine)}</div>
      ${resumo ? `<div style="margin-top:6px;">${escapeHtml(resumo)}</div>` : ""}
      <div class="insights-3">
        ${sec("✅ Pontos Positivos", positivos)}
        ${sec("⚠ Problemas Identificados", problemas)}
        ${sec("➡ Ações Recomendadas", recomendacoes)}
      </div>
      ${actionsHtml}
      <div class="diag-title" style="margin-top:16px;">🚨 <span>Alertas IA</span></div>
      ${itemsHtml}
    `;
  }
